...
namespace MarkdownMonster.WebSockets
{
    public class WebSocketServer
    {
        ...
        {
            if (!force && mmApp.Model.Window.WebSocketServer != null)
                return mmApp.Model.Window.WebSocketServer; // already running

            // if already running stop first then restart
            mmApp.Model.Window.WebSocketServer?.StopServer();

            var server = new WebSocketServer();
            server.OnBinaryMessage += OnBinaryMessageInternal;
            server.StartServer();
            mmApp.Model.Window.WebSocketServer = server;

            mmApp.Model.Configuration.WebSocket.IsRunning = true;

            return mmApp.Model.Window.WebSocketServer;
        }
        ...
        {
            var ws = mmApp.Model.Window.WebSocketServer;
            if (ws != null)
            {
                ws.OnBinaryMessage -= OnBinaryMessageInternal;
                ws.StopServer();
                mmApp.Model.Window.WebSocketServer = null;
            }
